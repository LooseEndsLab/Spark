...
struct FollowUpChecker {
    ...
    func findGhostedConversations(thresholdDays: Int, maximumAgeDays: Int? = nil, ignoredChatIDs: Set<Int64>, dismissedMessageIDs: Set<Int64>, ignoreGroupChats: Bool, includeGroupChatsInRespond: Bool = false, treatReactionsAsReplies: Bool = true, now: Date = .now) throws -> [FollowUp] {
        try findConversationStatuses(thresholdDays: thresholdDays, maximumAgeDays: maximumAgeDays, ignoredChatIDs: ignoredChatIDs, dismissedMessageIDs: dismissedMessageIDs, ignoreGroupChats: ignoreGroupChats, includeGroupChatsInRespond: includeGroupChatsInRespond, treatReactionsAsReplies: treatReactionsAsReplies, now: now).waitingOnYou
    }

    func findConversationStatuses(thresholdDays: Int, maximumAgeDays: Int? = nil, ignoredChatIDs: Set<Int64>, dismissedMessageIDs: Set<Int64>, ignoreGroupChats: Bool, includeGroupChatsInRespond: Bool = false, treatReactionsAsReplies: Bool = true, now: Date = .now) throws -> (waitingOnThem: [FollowUp], waitingOnYou: [FollowUp]) {
        ...
        let metadataCandidates = try store.latestConversationMessages()
            ...
            .filter { includeGroupChatsInRespond || !$0.isGroupChat || $0.isFromMe }
